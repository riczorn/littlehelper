...
class LittleHelperControllerHtaccess extends JControllerForm
{
	...
	/**
	 * Rename an .htaccess to a random backup name
	 */
	...
	/**
	 * Copy a sample .htaccess in the new place
	 */
	public function create() {
		$key = JFactory::getApplication()->input->get('key');
		if ($this->getModel()->create($key)) {
			$this->setRedirect(JRoute::_('index.php?option=com_littlehelper&view=htaccess', false),strtolower($key)."/.htaccess ".JText::_("COM_LITTLEHELPER_FILE_CREATED"));
		} else {
			$this->setRedirect(JRoute::_('index.php?option=com_littlehelper&view=htaccess', false));
		}
		$this->redirect();
	...
	public function createIndex() {
		...
		if ($this->getModel()->create($key,'index.html')) {
			$this->setRedirect(JRoute::_('index.php?option=com_littlehelper&view=htaccess', false),strtolower($key)."/index.html ".JText::_("COM_LITTLEHELPER_FILE_CREATED"));
		} else {
			$this->setRedirect(JRoute::_('index.php?option=com_littlehelper&view=htaccess', false));
		}
		$this->redirect();
	...
	/**
	 * Create the Joomla root .htaccess (copy htaccess.txt)
	 */
	public function createRoot() {
		$jinput = JFactory::getApplication()->input;
		$kind = $jinput->get('kind'); // which is insignificant here: this can only be invoked in the root.
	...
	/**
	 * Restore a backup of an .htaccess
	 */
	...
	public function testRoot() {
		$jinput = JFactory::getApplication()->input;
		$kind = $jinput->get('kind'); // which is insignificant here: this can only be invoked for the root.
		$symlinks = $jinput->get('symlinks','')=='remove';
