		...
		*/
		...
		$jamssPatterns = array(
				...
				array('preg_match\s*\(\s*\"\s*/\s*bot\s*/\s*\"',
						...
						array('\$\w[\w\W\s/\*]*=[\w\W\s/\*]*`[a-zA-Z0-9 _]+[\#\/:,;-].*`',
						...
						'command, which might indicate a part of a webrootkit. It could also '.
						'be an elaborate sql command, in which case it could be legitimate'
						),
		);
